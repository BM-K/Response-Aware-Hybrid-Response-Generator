...
def evaluate(ref_file, trans_file, ref_src_file, metric, fid=1, subword_option=None):
  ...
  if len(metric.lower()) > 4 and metric.lower()[0:4]=='bleu':
    ...
    evaluation_score = _entropy_nrg(dict_files, trans_file, fid, subword_option=subword_option)
  else:
...
def _entropy_nrg(dict_files, trans_file, fid, subword_option=None):
